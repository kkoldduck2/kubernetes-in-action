...
- **파드가 필요한 이유?**
    - 컨테이너는 단일 프로세스를 실행하는 것을 목적으로 설계했다.
        ...
        - 예) 개별 프로세스가 실패하는 경우 자동으로 재시작하는 매커니즘을 포함해야 함. 또한 이러한 모든 프로세스는 동일한 표준 출력으로 로그를 기록하기 때문에 어떤 프로세스가 남긴 로그인지 파악하는 것이 어렵다.
...
- 통합 모니터링에서 로그 수집 정책 바꾸는 이유
    - 시스템 로그는 기존 콘솔 로그에 남기고, 비즈니스 로그 처럼 민감한 정보는 노드에만 남기고자 함
    - 대신 더이상 kubelet에서 로테이트 및 삭제를 관리하지 않음. 직접 해줘야함. 따라서 로테이트 정책은 log4j 파일에, 삭제 정책은 elastic agent에 명시함
    - 로테이트 정책 뿐만 아니라 로그 쌓는 경로, 로그 파일명 역시 log4j에 명시되어 있음
    - gitops 설정으로 해당 log4j 파일이 위치하는 컨테이너 경로를 host path로 호스트 노드 경로로 매핑
    - elastic agent는 해당 호스트 노드 경로에 위치한 log 파일을 읽어서 elasticsearch로 보냄. 동시에 사이드 컨테이너로 일정 시간이 지난 백업 파일은 삭제하는 busy box 추가
    - 클러스터 레벨의 로깅이 가능하기 위해서 파일명에 container id가 포함되어 있어야 함. 근데 못해서 파일명에 pod id만 포함됨. 따라서 컨테이너 레벨의 모니터링 어려움
    - 다행히 비즈니스 로그에 대해서만 수행하고, 쿠버네티스 메트릭은 기존과 동일하게 수집하는거 맞나?!?!
